...
def run_benchmark_subprocess(name, args):
    print("Running ", name)
    proc = subprocess.run(
        shlex.join(["python", "peak_memory_cat_cont_runner.py"] + args),
        capture_output=True,
        text=True,
        shell=True
    )
    ...
    with open("process_cat_cont_memory.txt", 'r') as f:
        peak_mem = float(f.read())
    output_dict['memory'] = peak_mem

    # Delete memory result file.
    if os.path.exists("process_cat_cont_memory.txt"):
        os.remove("process_cat_cont_memory.txt")
...
if __name__ == "__main__":
    ...
    for i in range(NUM_RUNS):
        ...
        bin_data_nan = simulate_cat_data(NUM_FEATURES, NUM_SAMPLES, NA_RATIO, np.nan, max_categories=4)
        ...
        for num_threads in THREAD_LIST:
            # ANOVA methods.
            run_results = run_benchmark_subprocess(f'ANOVA NApyNumba NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_napy_path),
                                                    "--tool", "napy_numba", '--measure', "anova",
                                                    "--bin_file", str(bin_data_napy_path)])
            all_results = record_results(all_results, run_results, num_run=i)
            run_results = run_benchmark_subprocess(f'ANOVA NApyCPP NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_napy_path),
                                                    "--tool", "napy_cpp", '--measure', "anova",
                                                    "--bin_file", str(bin_data_napy_path)])
            all_results = record_results(all_results, run_results, num_run=i)
            run_results = run_benchmark_subprocess(f'ANOVA ScipyPar NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_nan_path),
                                                    "--tool", "scipy_par", '--measure', "anova",
                                                    "--bin_file", str(bin_data_nan_path)])
            all_results = record_results(all_results, run_results, num_run=i)

            # MWU methods.
            run_results = run_benchmark_subprocess(f'KW NApyNumba NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_napy_path),
                                                    "--tool", "napy_numba", "--bin_file", str(bin_data_napy_path),
                                                    '--measure', "kw"])
            all_results = record_results(all_results, run_results, num_run=i)
            run_results = run_benchmark_subprocess(f'KW NApyCPP NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_napy_path),
                                                    "--tool", "napy_cpp", '--measure',
                                                    "kw", "--bin_file", str(bin_data_napy_path)])
            all_results = record_results(all_results, run_results, num_run=i)
            run_results = run_benchmark_subprocess(f'KW ScipyPar NUM={i}',
                                                   ["--threads", f'{num_threads}', "--cont_file", str(cont_data_nan_path),
                                                    "--tool", "scipy_par",
                                                    '--measure', "kw", "--bin_file", str(bin_data_nan_path)])
            all_results = record_results(all_results, run_results, num_run=i)
    ...
    results_df.to_csv('peak_memory_cat_cont_results.tsv', sep='\t')
