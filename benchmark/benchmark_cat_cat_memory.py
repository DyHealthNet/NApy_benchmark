...
if __name__ == "__main__":
    ...
    _, _ = calculate_scipy_chi2(test_data, 2)
            ...
            for nr_samples in sample_list:
                for i in range(NUM_RUNS):
                    ...
                    for nr_threads in thread_list:
                        with memray.Tracker(file_dir + f'/nanpy_chi2_{nr_features}_{nr_samples}_{na_ratio}_{nr_threads}_{i}.bin', follow_fork=True, native_traces=False):
                            res, nanpy_time = nanpy_chi2_wrapper(data_np, NA_VALUE, nr_threads)

                        with memray.Tracker(file_dir + f'/numba_chi2_{nr_features}_{nr_samples}_{na_ratio}_{nr_threads}_{i}.bin', follow_fork=True, native_traces=False):
                            res_numba, numba_time = nanpy_chi2_numba_wrapper(data_np, NA_VALUE, nr_threads)

                        with memray.Tracker(file_dir + f'/scipypar_chi2_{nr_features}_{nr_samples}_{na_ratio}_{nr_threads}_{i}.bin', follow_fork=True, native_traces=False):
