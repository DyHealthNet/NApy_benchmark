...
ggplot(tmp[tmp$samples == 1000 & tmp$features == 1000,], aes(x = as.factor(threads), y = mean_mem, fill = library)) + 
  ...
  facet_wrap(~method, ncol = 7, label = "label_parsed", scales = "free_y") + 
  theme_bw() + scale_fill_manual(values = library_colors) + labs(x = "Threads", y = "Mean Peak Resident Memory (MB)", fill = "Library") + theme_new + theme(strip.background = element_rect(fill = "white"), legend.position = "bottom")

ggsave(paste0(out_dir, "peak_memory_napy_numba_c++.png"), width = 15, height = 4, dpi = 300)
...
```{r}
p <- ggplot(tmp, aes(x = library, y = mean_mem, fill = threads)) + geom_bar(stat="identity", position = "dodge") + facet_grid(.~method, scales="free", space = "free", label = "label_parsed") + theme_bw() + labs(x = "Library", y= "Mean Peak Resident Memory (MB)", fill = "Threads") + scale_fill_manual(values = c("#ccd7ed","#99b0db", "#4d74c1", "#0039ab")) + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + theme(strip.background = element_rect(fill = "white"), legend.position = "bottom") + guides(fill = guide_legend(nrow = 1))
p
...
```{r}
p_thread_1 <- ggplot(tmp[tmp$threads == 1,], aes(x = library, y = mean_mem, fill = library)) + geom_bar(stat = "identity") + facet_grid(.~method, scales = "free_x", space = "free", label = "label_parsed") + theme_bw() + scale_fill_manual(values = library_colors) + labs(x = "Library", y = "Mean Peak Resident Memory (in MB)", fill = "Library") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(strip.background = element_rect(fill = "white"), legend.position = "bottom") +  + guides(fill = guide_legend(nrow = 1))
p_thread_1     
```

```{r}
p_threads <- ggplot(memory_dt[!memory_dt$library %in% c("Pingouin", "pandas", "SciPy"),], aes(x = library, y = mean_mem, fill = threads)) + geom_bar(stat = "identity", position = "dodge") + facet_grid(.~method, scales = "free_x", space = "free") + theme_bw() + labs(x = "Library", y = "Mean Peak Memory (in MB)", fill = "Threads") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_fill_brewer(palette = "Blues")
p_threads
```


```{r}
p_thread_1 / p_threads


```

```{r}
mem_table <- as.data.table(memory_dt[, c("threads", "library", "method", "mean_mem")])
mem_table$mean_mem <- round(mem_table$mean_mem, 3)
mem_table_wide <- dcast(mem_table, library + threads ~ method, value.var = "mean_mem")
write.csv(mem_table_wide, paste0(out_dir, "peak_memory.csv"))
