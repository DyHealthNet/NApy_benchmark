...
runtimes_CHRIS_sum_scipy <- runtimes_CHRIS[! runtimes_CHRIS$method %in% c("Pearson", "Spearman"),] %>% group_by(type1, type2, threads, method) %>% reframe(fold_change_comp_napy = mean_time[library == "scipypar"] / mean_time[library == "nanpy"],
                                                                                                                                                           fold_change_napy_comp = mean_time[library == "nanpy"] / mean_time[library == "scipypar"])

runtimes_CHRIS_sum_pandas <- runtimes_CHRIS[runtimes_CHRIS$method %in% c("Pearson", "Spearman"),] %>% group_by(type1, type2, threads, method) %>% reframe(fold_change_comp_napy = mean_time[library == "pandas"] / mean_time[library == "nanpy"],
                                                                                                                                                          fold_change_napy_comp = mean_time[library == "nanpy"] / mean_time[library == "pandas"])
...
runtimes_CHRIS_sum$log_fold_change <- log10(runtimes_CHRIS_sum$fold_change_comp_napy)

runtimes_CHRIS_sum$label <- paste0(round(runtimes_CHRIS_sum$fold_change_comp_napy, 2), " / 1")

runtimes_CHRIS_sum$label[runtimes_CHRIS_sum$method == "Pearson" & runtimes_CHRIS_sum$threads == 1] <- paste0("1 / ", round(runtimes_CHRIS_sum$fold_change_napy_comp[runtimes_CHRIS_sum$method == "Pearson" & runtimes_CHRIS_sum$threads == 1], 2))


ggplot(runtimes_CHRIS_sum, aes(x = threads, y = method, fill = log_fold_change, label = label)) + 
  geom_tile(color ="black") + 
  geom_label(color = "black", fill = "white", alpha = 0.7) + 
  labs( x = "Number of Threads", y = "Statistical Test", fill = expression(paste(Log[10], "(Runtime Fastest Competitor / Runtime NApy)"))) +
