...
```{r}
NA_bin <- read_missing_values_files(paste0(input_dir, "chris_bin_missing.csv"), type = "Binary Categorical")
...
NA_all$Type <- factor(NA_all$Type, levels = c("Continuous", "Binary Categorical", "Categorical"))
```
...
ggplot(runtimes_CHRIS_sum, aes(x = threads, y = method, fill = log_fold_change, label = round(fold_change, 2))) + 
  ...
  labs( x = "Number of Threads", y = "Statistical Test", fill = "Log10 Runtime Fold Change") + theme_bw() + theme_new + theme(legend.position= "top") +
  guides(fill = guide_colorbar(
    title.position = "left",
    title.vjust = 0.8,
    barwidth = unit(5, "cm")
)) + scale_fill_gradient2(low = "#B2182B", high = "#2166AC", mid = "white", midpoint = 0, limits = c(-max(abs(runtimes_CHRIS_sum$log_fold_change)), max(abs(runtimes_CHRIS_sum$log_fold_change))))
