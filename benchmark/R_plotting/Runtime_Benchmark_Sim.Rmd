...
tmp$winner_label <- paste0("Numba x ", round(tmp$fold_change_c_numba, 2))
tmp$winner_label[tmp$method %in% c("Spearman", "Mann-Whitney-U", "Kruskal-Wallis")] <- paste0("C++ x ",  round(tmp$fold_change_numba_c[tmp$method %in% c("Spearman", "Mann-Whitney-U", "Kruskal-Wallis")],2))
  

tmp$label <- paste0(round(tmp$fold_change_c_numba,2), " (", round(tmp$fold_change,2), ")")

ggplot(tmp, aes(x = threads, y = method, fill = log10(fold_change_c_numba), label = winner_label)) + 
...
ggplot(tmp, aes(x = threads, y = method, fill = fold_change_c_numba, label = round(fold_change_c_numba, 2))) + 
  geom_tile(color ="black") + geom_label(color = "black", fill = "white", alpha = 0.7) + 
  theme_bw() + labs(x = "Number of Threads", y = "Statistical Test", fill =  "Runtime C++ / Runtime Numba") + 
  guides(color = FALSE)  + 
  scale_fill_gradient(low = "white", high = "#013870", 
                       limits = c(0, max(abs(tmp$fold_change_c_numba)))) + 
  theme_new + theme(legend.position= "top") +
  scale_y_discrete(labels = function(labels) {
  ifelse(
    labels == "Chi-squared", expression(chi^2 ~ "-Test"),
...
thread_plot <- ggplot(runtime_threads_recoded,
  ...
  labs(x = "Number of Threads", y = "Log10 Mean Runtime (s)", color = "Library", fill = "Library") +
  theme_new +
...
plot_magnitude_heatmap <- function(dt, column = "threads", xlab = "Number of Threads"){
  ...
  rt$label <- paste0("x ", round(rt$fold_change, 2) )
...
samples_plot <- ggplot(runtime_samples_features_recoded[runtime_samples_features_recoded$features == 1000,],
  ...
  scale_color_manual(values = library_colors) +
  scale_fill_manual(values = library_colors) +
  labs(x = "Number of Samples", y = "Log10 Mean Runtime (s)", color = "Library", fill = "Library") +
  theme_new
...
features_plot <- ggplot(runtime_samples_features_recoded[runtime_samples_features_recoded$samples == 1000,],
  ...
  scale_color_manual(values = library_colors) +
  scale_fill_manual(values = library_colors) +
  labs(x = "Number of Features", y = "Log Mean Runtime (s)", color = "Library", fill = "Library") +
  theme_new
  #theme(legend.position = "bottom") + theme_new
