...
## ----- Theme -----
theme_new <- theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14), legend.text = element_text(size = 12), legend.title = element_text(size = 14), strip.text = element_text(size = 14), legend.position = "top")
...
ggplot(runtime_dt_napy[runtime_dt_napy$samples == 1000 & runtime_dt_napy$method %in% c("Pearson", "Spearman"),], aes(x = features, y = mean_time, color = library, shape = threads, group = interaction(library, threads))) + 
  geom_point(size = 2.5) + geom_line(aes(linetype = threads)) + geom_errorbar(aes(ymin = mean_time - sd_time, ymax = mean_time + sd_time), width = 0.2) +
  facet_wrap(~method, scales = "free_y", ncol = 4) + 
  theme_bw() + scale_color_manual(values = library_colors) + labs(x = "Number of Features", y = "Mean Runtime (s)", color = "Library", shape = "Threads", linetype = "Threads") + theme_new

ggsave(paste0(out_dir, "pear_spear_C_numba.png"), width = 8, height = 4, dpi = 300)
...
ggsave(paste0(out_dir, "others_C_numba.png"), width = 8, height = 11, dpi = 300)
```
...
ggplot(tmp, aes(x = threads, y = method, fill = log10(fold_change), label = round(fold_change, 2))) + geom_tile(color ="black") + geom_label(color = "black", fill = "white", alpha = 0.7) + theme_bw() + labs(x = "Number of Threads", y = "Statistical Test", fill =  "Log10 Runtime Fold Change") + guides(color = FALSE)  + scale_fill_gradient2(low = "#B2182B", high = "#2166AC", mid = "white", midpoint = 0, limits = c(-max(abs(log10(tmp$fold_change))), max(abs(log10(tmp$fold_change))))) + theme_new + theme(legend.position= "top") +
...
thread_plot <- ggplot(runtime_threads,
  ...
  facet_wrap(~method, scales = "free_y", ncol = 7) + 
  theme_bw() + 
...
plot_magnitude_heatmap <- function(dt, column = "threads", xlab = "Number of Threads"){
  ...
  p <- ggplot(rt, aes(x = get(column), y = method, fill = log_fold_change, label = round(fold_change, 2))) + 
    geom_tile(aes(x = get(column), y = method), color ="black") + 
    scale_fill_gradient(low = "white", high = "#2166AC",
                         limits = c(0, 3.2), 
                         name = "Log10 Runtime Fold Change",
...
samples_plot <- ggplot(runtime_samples_features[runtime_samples_features$features == 1000,],
...
```{r}
features_plot <- ggplot(runtime_samples_features[runtime_samples_features$samples == 1000,],
...
```{r}
(features_plot + heatmap_features + plot_layout(widths = c(5,1))) / 
  (samples_plot + heatmap_samples + plot_layout(widths = c(5,1))) / 
  (thread_plot + heatmap_threads + plot_layout(widths = c(5,1)))  + 
  plot_layout(guides = "collect") + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 20, face = "bold"), legend.position = "bottom", strip.background = element_rect(fill = "white")) & guides(color = guide_legend(nrow = 1))


(features_plot / samples_plot / thread_plot + plot_layout(guides = "collect") & theme(legend.position = "top") & guides(color = guide_legend(nrow = 1))) / (heatmap_features + heatmap_samples + heatmap_threads + plot_layout(axes = "collect", axis = "collect", guides = "collect") & theme(legend.position = "bottom")) + plot_layout(heights = c(1,1,1,1.2)) + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 20, face = "bold"), strip.background = element_rect(fill = "white"))
...
ggplot(na_runtime_dt_napy, aes(x = na_ratio, y = mean_time, color = library, group = library)) + geom_point(size = 3) + 
  facet_wrap(~method, scales = "free_y", ncol = 2) + geom_line() +
  geom_errorbar(aes(ymin = mean_time - sd_time, ymax = mean_time + sd_time), width = 0.02) +
  theme_bw() + theme_new + scale_color_manual(values = library_colors, name = "Library") +
  labs(x = "Missing Value Ratio", y = "Mean Runtime (s)")
...
```{r}
ggplot(na_runtime_dt[na_runtime_dt$method %in% c("Pearson", "Spearman"),], aes(x = na_ratio, y = mean_time, color = library)) + geom_point(size = 3) + 
  facet_wrap(~method, scales = "free_y", ncol = 2) + geom_line() +
  geom_errorbar(aes(ymin = mean_time - sd_time, ymax = mean_time + sd_time), width = 0.02) +
  theme_bw() + theme_new + scale_color_manual(values = library_colors, name = "Library") +
  labs(x = "Missing Value Ratio", y = "Mean Runtime (s)") + theme(legend.position = "none")

ggsave(paste0(out_dir, "na_ratio_pearson_spearman.png"), width = 8, height = 4, dpi = 300)
...
ggsave(paste0(out_dir, "na_ratio_others.png"), width = 8, height = 11, dpi = 300)
```

Alternative way:

```{r}
#na_runtime_dt$na_ratio <- as.factor(na_runtime_dt$na_ratio)
ggplot(na_runtime_dt, aes( x = na_ratio, y = method, fill = log10(mean_time), label = round(mean_time, 2))) + geom_tile() + geom_label()
...
ggplot(na_runtime_dt, aes(x = na_ratio, y = mean_time, color = method)) +  geom_point(size = 3, alpha = 0.7) + geom_line(aes(group = method), alpha = 0.7) + geom_errorbar(aes(ymin = mean_time - sd_time, ymax = mean_time + sd_time), width = 0.02) +
  theme_bw() +
  scale_color_brewer(palette = "Paired") + labs(x = "Missing Value Ratio", y = "Mean Runtime (s)", color = "Statistical Test") + theme_new + theme(legend.position = "right")
