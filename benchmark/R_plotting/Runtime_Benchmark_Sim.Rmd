...
ggplot(tmp, aes(x = threads, y = method, fill = log10(fold_change), label = round(fold_change, 2))) + 
...
Thread Plot Only NApy

```{r}
thread_plot_napy <- ggplot(runtime_threads_recoded[runtime_threads_recoded$library == "NApy",],
...
plot_magnitude_heatmap <- function(dt, column = "threads", xlab = "Number of Threads"){
  ...
  p <- ggplot(rt, aes(x = get(column), y = method, fill = log10(fold_change), label = round(fold_change, 2))) + 
...
x <- xtable(runtime_recoded, align = c("|c","|c","|c","|c","|c", "|c", "|c", "|c", "|c", "|c", "|c|"))
print(x, include.row.names = FALSE)
